...
//STEP 3-----------------
//We would like our utilities.js collection to be a reusable module. So we need to export our 
//helper functions to be used elsewhere. Add an export keyword before each of our functions to export them.
...
async function showQuakes() {
    ...
    const location = await myQuakesController.initPos();
    // get the list of quakes for the location
    const quakes = await myQuakesController.getQuakesForLocation(location);
    // render the list to the list element
    const listElement = document.querySelector("#quakeList");
    listElement.innerHTML = myQuakesController.generateListMarkup(
      quakes.features,
...
//STEP 8-------------------------
//Located in QuakesController.js
